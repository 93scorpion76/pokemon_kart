...
public class CarControlCS : MonoBehaviour {
    ...
    void stabilize()
    {
            ...
            tmpPos.y -= 100;
            Ray ray = new Ray(this.transform.position, tmpPos);
            RaycastHit hitInfo = new RaycastHit();

            Physics.Raycast(ray, out hitInfo);
        if (this.transform.position.y - hitInfo.point.y > 0.1f)
        {
            int rotationStabilize;
            ...
            transform.Rotate(transform.rotation.x, transform.rotation.y, transform.rotation.z + rotationStabilize);
        }
     }
